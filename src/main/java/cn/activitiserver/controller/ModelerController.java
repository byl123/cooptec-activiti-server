package cn.activitiserver.controller;
...
public class ModelerController{
	...
	public ModelAndView defaultIndex(ModelAndView modelAndView) {
		modelAndView.setViewName("modelsView/index");
		modelAndView.addObject("modelList", repositoryService.createModelQuery().list());
		modelAndView.addObject("processList",repositoryService.createDeploymentQuery().list());
		return modelAndView;
	}
    ...
    public void create(HttpServletResponse response, String name, String key) throws IOException {
        ...
        response.sendRedirect("/model/editor?modelId="+ model.getId());
	 ...
	 * @param modelId
	 */
	@RequestMapping(value = "createdProcessImage")
	public void showModelPicture(HttpServletResponse response ,String modelId) {
		...
		InputStream inputStream = processDiagramGenerator.generateDiagram(model, "PNG",
				...
				processEngineConfiguration.getProcessEngineConfiguration().getActivityFontName(),
				processEngineConfiguration.getProcessEngineConfiguration().getProcessEngineConfiguration().getClassLoader(), 1);
		OutputStream out = null;
		try {
			// 配置文件下载
			response.setHeader("content-type", "application/octet-stream");
			response.setContentType("application/octet-stream");
			response.setHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(modelData.getName()+".png", "UTF-8"));
